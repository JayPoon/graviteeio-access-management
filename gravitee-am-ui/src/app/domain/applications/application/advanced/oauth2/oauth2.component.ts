...
export class ApplicationOAuth2Component implements OnInit {
  ...
  initCustomGrantTypes() {
    this.customGrantTypes.forEach(customGrantType => {
      ...
      customGrantType.checked = _.some(this.applicationOauthSettings.grantTypes, authorizedGrantType => authorizedGrantType.toLowerCase() === customGrantType.value.toLowerCase());
    });
  }
  ...
  initScopes() {
    ...
    this.selectedScopes = _.map(this.applicationOauthSettings.scopes, function(scope) {
  ...
  addClaim(claim) {
    if (claim) {
      if (!this.claimExits(claim.tokenType, claim.claimName)) {
        claim.id = Math.random().toString(36).substring(7);
        this.application.tokenCustomClaims.push(claim);
        this.application.tokenCustomClaims = [...this.application.tokenCustomClaims];
        this.formChanged = true;
      } else {
  ...
  updateClaim(tokenType, event, cell, rowIndex) {
    ...
    if (claim) {
      ...
      let index = _.findIndex(this.application.tokenCustomClaims, {id: rowIndex});
