...
public class ApplicationServiceTemplate implements ApplicationTemplate {
    ...
    public void handle(Application application) {
        ...
        // service applications must have client_credentials flow and no response types
        oAuthSettings.setGrantTypes(Collections.singletonList(GrantType.CLIENT_CREDENTIALS));
        oAuthSettings.setResponseTypes(null);
        // no need user account and advanced configuration for service applications
        application.getSettings().setAccount(null);
        application.getSettings().setAdvanced(null);
        // no identity providers for service application
        application.setIdentities(null);
    }
}
